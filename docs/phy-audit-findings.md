...
`sionna.phy` case set. It combines the channel, mapping, signal, and standalone
OFDM cases currently implemented in this repository.

The important point is that all 75 current dynamic PHY cases show device
...
- Total current PHY dynamic cases: 75.
- Failed audit cases: 75.
- Skipped cases: 0.
...
Extend dynamic coverage to standalone `sionna.phy.mimo` cases. After those
cases are implemented, run:
